...
from pyhelios import Context, WeberPennTree, WPTType, RadiationModel, RadiationModelError
...
def main():
    ...
    try:
        ...
        tree_id = wpt.buildTree(WPTType.LEMON, 
                               recursion_depth=3,     # Control tree complexity
                               branch_subdivisions=6, # Smooth branches
                               leaf_subdivisions=4)   # Detailed leaves
        ...
        print(f"  Tree generated with {len(leaf_uuids)} leaves, {len(branch_uuids)} branches")
        ...
        with RadiationModel(context) as radiation:
            ...
            print(f"  Configured {len(bands)} radiation bands with realistic solar spectrum")
            print(f"  Ray tracing quality: {quality_settings['direct_rays']} direct, {quality_settings['diffuse_rays']} diffuse rays")
            ...
            nir_filename = radiation.write_norm_camera_image(
            ...
            print(f"  🎯 Object detection: {output_dir}/plant_classes.txt + YOLO labels")
            ...
            algorithms = [
                ...
                ("MATRIX_3X3_AUTO", "Full matrix with stability fallback (recommended)"),
            ...
            for algorithm, description in algorithms:
                try:
                    calibrated_filename = radiation.auto_calibrate_camera_image(
                        ...
                        algorithm=algorithm,
                        print_quality_report=(algorithm == "MATRIX_3X3_AUTO"),  # Report for recommended algorithm
                        ccm_export_file_path=f"{output_dir}/ccm_{algorithm.lower()}.txt" if algorithm == "MATRIX_3X3_AUTO" else ""
                    )
        ...
        print("      python -c \"from pyhelios.plugins import get_plugin_registry; print(get_plugin_registry().get_available_plugins())\"")
