...
def main():
    ...
    quad_faces = np.array(
        [[0, 1, 2], [0, 2, 3]], dtype=np.int32  # first triangle  # second triangle
    )
    ...
    try:
        ...
        multi_textured_uuids = context.addTrianglesFromArraysTextured(
            ...
            texture_files=[
                "models/Helios_logo.jpeg",
                "models/Helios_logo.jpeg",
            ],  # Using same texture twice for demo
            material_ids=material_ids,
        )
        print(
            f"   Multi-textured triangles: {len(multi_textured_uuids)} triangles added"
        )
        print(f"   Multi-texture UUIDs: {multi_textured_uuids}")
    except Exception as e:
        print(f"   Multi-texture example skipped: {e}")
        print(
            "   (This is expected if texture files don't exist or native library isn't built)"
        )
    ...
    if array_uuids:
        ...
        print(
            f"     Normal: ({prim_info.normal.x:.3f}, {prim_info.normal.y:.3f}, {prim_info.normal.z:.3f})"
        )
        print(
            f"     Centroid: ({prim_info.centroid.x:.3f}, {prim_info.centroid.y:.3f}, {prim_info.centroid.z:.3f})"
        )
        print(
            f"     Color: RGB({prim_info.color.r:.3f}, {prim_info.color.g:.3f}, {prim_info.color.b:.3f})"
        )
        print(f"     Vertices:")
    ...
    print(
        "   Note: Primitive data methods require additional C++ wrapper implementation"
    )
    ...
    print(
        "   - temp = context.getPrimitiveData(uuid, 'temperature')  # Auto-detects type"
    )
    print(
        "   - temp = context.getPrimitiveData(uuid, 'temperature', float)  # Explicit type (optional)"
    )
