...
try:
    ...
    helios_lib.destroySkyViewFactorModel.argtypes = [
        ctypes.POINTER(USkyViewFactorModel)
    ]
    helios_lib.destroySkyViewFactorModel.restype = None

    # Message control
    helios_lib.disableSkyViewFactorMessages.argtypes = [
        ctypes.POINTER(USkyViewFactorModel)
    ]
    helios_lib.disableSkyViewFactorMessages.restype = None

    helios_lib.enableSkyViewFactorMessages.argtypes = [
        ctypes.POINTER(USkyViewFactorModel)
    ]
    helios_lib.enableSkyViewFactorMessages.restype = None

    # Ray count configuration
    helios_lib.setSkyViewFactorRayCount.argtypes = [
        ctypes.POINTER(USkyViewFactorModel),
        ctypes.c_uint,
    ]
    ...
    helios_lib.setSkyViewFactorMaxRayLength.argtypes = [
        ctypes.POINTER(USkyViewFactorModel),
        ctypes.c_float,
    ]
    helios_lib.setSkyViewFactorMaxRayLength.restype = None

    helios_lib.getSkyViewFactorMaxRayLength.argtypes = [
        ctypes.POINTER(USkyViewFactorModel)
    ]
    helios_lib.getSkyViewFactorMaxRayLength.restype = ctypes.c_float

    # Single point calculation
    helios_lib.calculateSkyViewFactor.argtypes = [
        ctypes.POINTER(USkyViewFactorModel),
        ctypes.c_float,
        ctypes.c_float,
        ctypes.c_float,
    ...
    helios_lib.calculateSkyViewFactorCPU.argtypes = [
        ctypes.POINTER(USkyViewFactorModel),
        ctypes.c_float,
        ctypes.c_float,
        ctypes.c_float,
    ...
    helios_lib.calculateSkyViewFactors.argtypes = [
        ctypes.POINTER(USkyViewFactorModel),
    ...
    helios_lib.calculateSkyViewFactorsForPrimitives.argtypes = [
        ctypes.POINTER(USkyViewFactorModel),
        ctypes.POINTER(ctypes.c_float),
    ...
    helios_lib.exportSkyViewFactors.argtypes = [
        ctypes.POINTER(USkyViewFactorModel),
        ctypes.c_char_p,
    ]
    helios_lib.exportSkyViewFactors.restype = ctypes.c_bool

    helios_lib.loadSkyViewFactors.argtypes = [
        ctypes.POINTER(USkyViewFactorModel),
        ctypes.c_char_p,
    ]
    ...
    # Get results
    helios_lib.getSkyViewFactors.argtypes = [
        ctypes.POINTER(USkyViewFactorModel),
        ctypes.POINTER(ctypes.c_size_t),
    ]
    helios_lib.getSkyViewFactors.restype = ctypes.POINTER(ctypes.c_float)

    helios_lib.getSamplePoints.argtypes = [
        ctypes.POINTER(USkyViewFactorModel),
        ctypes.POINTER(ctypes.c_size_t),
    ]
    helios_lib.getSamplePoints.restype = ctypes.POINTER(ctypes.c_float)

    # Statistics
    helios_lib.getSkyViewFactorStatistics.argtypes = [
        ctypes.POINTER(USkyViewFactorModel)
    ]
    helios_lib.getSkyViewFactorStatistics.restype = ctypes.c_char_p

    # CUDA/OptiX availability
    helios_lib.isSkyViewFactorCudaAvailable.argtypes = [
        ctypes.POINTER(USkyViewFactorModel)
    ]
    helios_lib.isSkyViewFactorCudaAvailable.restype = ctypes.c_bool

    helios_lib.isSkyViewFactorOptiXAvailable.argtypes = [
        ctypes.POINTER(USkyViewFactorModel)
    ]
    ...
    helios_lib.destroySkyViewFactorCamera.argtypes = [
        ctypes.POINTER(USkyViewFactorCamera)
    ]
    helios_lib.destroySkyViewFactorCamera.restype = None

    # Camera configuration
    helios_lib.setSkyViewFactorCameraPosition.argtypes = [
        ctypes.POINTER(USkyViewFactorCamera),
        ctypes.c_float,
        ctypes.c_float,
        ctypes.c_float,
    ...
    helios_lib.setSkyViewFactorCameraTarget.argtypes = [
        ctypes.POINTER(USkyViewFactorCamera),
        ctypes.c_float,
        ctypes.c_float,
        ctypes.c_float,
    ...
    helios_lib.setSkyViewFactorCameraUp.argtypes = [
        ctypes.POINTER(USkyViewFactorCamera),
        ctypes.c_float,
        ctypes.c_float,
        ctypes.c_float,
    ...
    helios_lib.setSkyViewFactorCameraFieldOfView.argtypes = [
        ctypes.POINTER(USkyViewFactorCamera),
        ctypes.c_float,
    ]
    helios_lib.setSkyViewFactorCameraFieldOfView.restype = None

    helios_lib.setSkyViewFactorCameraResolution.argtypes = [
        ctypes.POINTER(USkyViewFactorCamera),
        ctypes.c_uint,
        ctypes.c_uint,
    ]
    ...
    helios_lib.setSkyViewFactorCameraRayCount.argtypes = [
        ctypes.POINTER(USkyViewFactorCamera),
        ctypes.c_uint,
    ]
    helios_lib.setSkyViewFactorCameraRayCount.restype = None

    helios_lib.setSkyViewFactorCameraMaxRayLength.argtypes = [
        ctypes.POINTER(USkyViewFactorCamera),
        ctypes.c_float,
    ]
    ...
    # Camera rendering
    helios_lib.renderSkyViewFactorCamera.argtypes = [
        ctypes.POINTER(USkyViewFactorCamera)
    ]
    helios_lib.renderSkyViewFactorCamera.restype = ctypes.c_bool

    # Camera results
    helios_lib.getSkyViewFactorCameraImage.argtypes = [
        ctypes.POINTER(USkyViewFactorCamera),
        ctypes.POINTER(ctypes.c_size_t),
    ]
    helios_lib.getSkyViewFactorCameraImage.restype = ctypes.POINTER(ctypes.c_float)

    helios_lib.getSkyViewFactorCameraPixelValue.argtypes = [
        ctypes.POINTER(USkyViewFactorCamera),
        ctypes.c_uint,
        ctypes.c_uint,
    ]
    ...
    helios_lib.exportSkyViewFactorCameraImage.argtypes = [
        ctypes.POINTER(USkyViewFactorCamera),
        ctypes.c_char_p,
    ]
    ...
    # Camera statistics
    helios_lib.getSkyViewFactorCameraStatistics.argtypes = [
        ctypes.POINTER(USkyViewFactorCamera)
    ]
    helios_lib.getSkyViewFactorCameraStatistics.restype = ctypes.c_char_p

    # Camera reset
    helios_lib.resetSkyViewFactorCamera.argtypes = [
        ctypes.POINTER(USkyViewFactorCamera)
    ]
...
def calculateSkyViewFactors(skyviewfactor_model, points, num_threads=0):
    ...
    # Call C++ function with num_threads parameter
    helios_lib.calculateSkyViewFactors(
        skyviewfactor_model, points_array, num_points, results_array, num_threads
    )
...
def calculateSkyViewFactorsForPrimitives(skyviewfactor_model, uuids, num_threads=0):
    """Calculate sky view factors for specified primitive centers"""
    if not _SKYVIEWFACTOR_MODEL_FUNCTIONS_AVAILABLE:
        raise RuntimeError(
            "SkyViewFactorModel functions are not available. Native library missing or skyviewfactor plugin not enabled."
        )
    ...
    # Convert UUIDs to C array
    num_primitives = len(uuids)
    uuids_array = (ctypes.c_uint * num_primitives)()
    for i, uuid in enumerate(uuids):
        uuids_array[i] = int(uuid)

    # Create output array
    results_array = (ctypes.c_float * num_primitives)()

    # Call C++ function
    helios_lib.calculateSkyViewFactorsForPrimitives(
        skyviewfactor_model, results_array, uuids_array, num_primitives, num_threads
    )
...
def exportSkyViewFactors(skyviewfactor_model, filename):
    ...
    return helios_lib.exportSkyViewFactors(
        skyviewfactor_model, filename.encode("utf-8")
    )
...
def destroySkyViewFactorCamera(camera):
    ...
    if not _SKYVIEWFACTOR_MODEL_FUNCTIONS_AVAILABLE:
        raise RuntimeError(
            "SkyViewFactorCamera functions are not available. Native library missing or skyviewfactor plugin not enabled."
        )
